...
void _vertexTransformAndAssembly(PrimitiveDevBufPointers primitive, glm::mat4 MVP, glm::mat4 MV, glm::mat3 MV_normal, glm::mat3 M_InverseTranspose, glm::vec3 eyePos, int width, int height)
{
    ...
    if (vid < numVertices)
    {
        ...
        for (int instanceId = 0; instanceId < numInstances; ++instanceId)
        {
            ...
            {
                primitive.dev_verticesOut[vid * numInstances + instanceId].color = glm::vec3(1.0f);//glm::clamp(glm::vec3(outPos), glm::vec3(0.f), glm::vec3(1.f));
            }
        }
    }
}
                            ...
                            if (isSet)
                            {
                                if (currDepth < dev_depth[pixelIndex])
                                {
                                    ...
                                    if (dev_diffuseTex != NULL)
                                    {
                                        ...
                                        dev_fragmentBuffer[pixelIndex].color = sampleTextureBiLinear(dev_diffuseTex, u, v, glm::fract(textureCoord), textureWidth, BYTES_PER_PIXEL);
#else
                                        dev_fragmentBuffer[pixelIndex].color = sampleTextureSimple(dev_diffuseTex, u, v, textureWidth, BYTES_PER_PIXEL);
#endif
                                    }
                                    else
                                    {
        ...
        else if (test == Line)
        {
            ...
            for (int row = minY; row <= maxY; ++row)
            {
                for (int col = minX; col <= maxX; ++col)
                {
                    ...
                    // Check if point is inside triangle
                    const bool isInside = isBarycentricCoordOnBoundary(baryCoord);
        ...
        else if (test == Point)
        {
            ...
            for (int row = minY; row <= maxY; ++row)
            {
                for (int col = minX; col <= maxX; ++col)
                {
                    ...
                    // Check if point is on the vertices of the triangle
                    const bool isInside = isBarycentricCoordOnVertices(triangle, currPos);
...
__global__
void _rasterizePrimitiveWithMSAA(int width, int height, int totalNumPrimitives, Primitive* dev_primitives, Fragment* dev_fragmentBuffer, float* dev_depth, unsigned int* mutexLock)
{
    // primitive id
    /*const int primitiveId = (blockIdx.x * blockDim.x) + threadIdx.x;
    ...
    if (primitiveId < totalNumPrimitives)
    {
        ...
        if (primitive.primitiveType == Triangle)
        {
            ...
            for (int row = minY; row <= maxY; row += MSAA_LEVEL)
            {
                for (int col = minX; col <= maxX; col += MSAA_LEVEL)
                {
